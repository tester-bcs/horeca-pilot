...
impl Parser {
    ...
    fn parse_body(
        ...
        for slot in spec.slots {
            if slot.optional && self.peek().value != slot.keyword {
                // Отсутствует в исходнике — не ошибка авторства (Ядро п.2
                // по-прежнему держит фиксированный ПОРЯДОК, но не требует
                // присутствия), портировано из Python (ast_parser.py):
                // подставляем default_value(), не требуя ключевого слова,
                // не трогая позицию курсора для следующего слота.
                slots.insert(slot.name, slot.default_value());
                continue;
            }
