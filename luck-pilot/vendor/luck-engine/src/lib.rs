...
mod tests {
    ...
    /// Портировано из Python (RESEARCH_BRIEF, находка 2 и 4 — POLICY/
    /// VERIFY/BUDGET существовали только на Python-стороне до слияния
    /// canonical, luck/registry.py): те же три optional-слота теперь
    /// парсятся и на Rust-стороне, обратная совместимость сохранена.
    #[test]
    fn policy_verify_budget_optional_slots_parse() {
        let src = r#"
NODE role: ROLE [GENERATIVE]
  AS "ops engineer"
END
NODE page: TOOL [EXTERNAL]
  CALL page WITH service="oncall"
  POLICY require="confirmed"
END
NODE report: STEP [GENERATIVE]
  DO "summarize"
  VERIFY checker="not_empty"
  INTO result
END
NODE plan: SPAWN [GENERATIVE]
  PLAN "decompose"
  BUDGET nodes="2", depth="1"
  INTO subgraph
END
EDGES:
  role -> page
...
END
"#;
        let graph = parse(src).expect("POLICY/VERIFY/BUDGET must parse");
        assert!(graph.nodes["page"].slots.get("policy").is_some());
        assert!(graph.nodes["report"].slots.get("verify").is_some());
        assert!(graph.nodes["plan"].slots.get("budget").is_some());
        // TOOL без POLICY по-прежнему парсится (обратная совместимость)
        let src2 = r#"
NODE t: TOOL [EXTERNAL]
  CALL x WITH a="1"
END
EDGES:
END
"#;
        parse(src2).expect("TOOL without POLICY must still parse");
    }
...
NODE step{i}: STEP [GENERATIVE] DO \"handle case {i}\" INTO out{i} END"
    ...
    fn canonical_form_cost_is_measured_not_guessed() {
        ...
        for i in 0..N {
            ...
            slots.insert("verify", SlotData::Args(Vec::new()));
