...
pub fn node_surface_form(
    ...
    for slot in spec.slots {
        ...
        if slot.optional {
            // Отсутствовал в исходнике (default_value) — не печатаем
            // ключевое слово: та же логика, что уже была найдена и
            // исправлена в Python (luck/registry.py, node_surface_form),
            // иначе ARGS-слот с пустым значением даёт синтаксически
            // обрубленную строку, не разбираемую парсером обратно.
            let is_default = match value {
                SlotData::Args(pairs) => pairs.is_empty(),
                SlotData::Str(s) => s.is_empty(),
                SlotData::Ident(s) => s.is_empty(),
                SlotData::Cond(..) => false,
            };
            if is_default {
                continue;
