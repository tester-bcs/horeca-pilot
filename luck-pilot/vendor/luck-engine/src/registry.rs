...
pub struct Slot {
    ...
    pub name: &'static str,
    pub value: SlotValue,
    /// Слот может отсутствовать в исходнике — портировано из Python
    /// (luck/registry.py, Slot.optional). Позиция в `slots` всё равно
    /// фиксирована (Ядро п.2): опциональность — про присутствие, не
    /// про порядок. Default false — существующие регистрации без
    /// явного значения обратно совместимы дословно.
    pub optional: bool,
}

impl Slot {
    /// Значение слота, когда он опущен в исходнике (только для optional).
    /// Портировано из Python (Slot.default_value); все текущие
    /// optional-слоты (POLICY/VERIFY/BUDGET) — ARGS, поэтому только этот
    /// вариант реализован, остальные — как только появится реальный
    /// optional-слот другого типа.
    pub fn default_value(&self) -> SlotData {
        match self.value {
            SlotValue::Args => SlotData::Args(Vec::new()),
            SlotValue::String => SlotData::Str(String::new()),
            SlotValue::Ident => SlotData::Ident(String::new()),
            _ => unreachable!("default_value для {:?} не портирован — не нужен пока", self.value),
        }
    }
}
...
impl NodeKindSpec {
    ...
    /// Продукция EBNF для тела узла — Срез 1. Optional-слот заключён в
    /// `[ ... ]` (EBNF: ноль или один раз) — та же нотация, что уже
    /// принимает GBNF-транслятор (luck/gbnf.py, портировано туда же
    /// вместе с этим слотом), не альтернативная своя.
    pub fn ebnf_production(&self) -> String {
        let parts: Vec<String> = self
            .slots
            .iter()
            .map(|s| {
                let part = format!("\"{}\" , {}", s.keyword, Self::value_symbol(s));
                if s.optional { format!("[ {part} ]") } else { part }
            })
            .collect();
        format!(
            "{}_body = {} ;",
...
macro_rules! slots {
    (@one ($kw:expr, $name:expr, $val:expr)) => {
        Slot { keyword: $kw, name: $name, value: $val, optional: false }
    };
    (@one ($kw:expr, $name:expr, $val:expr, optional)) => {
        Slot { keyword: $kw, name: $name, value: $val, optional: true }
    };
    ($($tup:tt),* $(,)?) => {
        &[$(slots!(@one $tup)),*]
    };
...
pub static REGISTRY: LazyLock<BTreeMap<Kind, NodeKindSpec>> = LazyLock::new(|| {
    ...
    put(NodeKindSpec {
        ...
        slots: slots![
            ...
            ("VERIFY", "verify", SlotValue::Args, optional),
        ],
        doc: "Задача: из данного входа произвести описанный результат. VERIFY — \
              см. STEP, тот же контракт.",
        output_ebnf: Some(OutputEbnf::Literal(
            "output = text ; text = char , { char } ;",
        )),
        stop_tokens: &["\n\n"],
        normalized_slots: &["verify"],
    });
    ...
    put(NodeKindSpec {
        ...
        doc: "Шаг пайплайна с именованным результатом. VERIFY (checker=\"...\", \
              field=\"...\", against=<DOCUMENT-узел>) — портировано из Python \
              (luck/registry.py): внешняя верификация вывода детерминированным \
              предикатом, не оценочным суждением модели о себе.",
        output_ebnf: Some(OutputEbnf::Literal(
    ...
    put(NodeKindSpec {
        ...
        slots: slots![
            ...
            ("BUDGET", "budget", SlotValue::Args, optional),
            ("INTO", "into", SlotValue::Ident),
        ],
        doc: "Порождающий узел: его вывод разбирается планировщиком как объявления \
              узлов Luck и вставляется в граф. Замыкает цикл 6->1. BUDGET \
              (nodes=N, depth=D) — локальное сужение глобального предела роста, \
              эффективный предел = min(глобальный, локальный).",
        output_ebnf: Some(OutputEbnf::NodeGrammar),
        stop_tokens: &["\n\n\n"],
        normalized_slots: &["budget"],
    });
    ...
    put(NodeKindSpec {
        ...
        slots: slots![
            ...
            ("POLICY", "policy", SlotValue::Args, optional),
        ],
        doc: "Вызов внешнего инструмента. Аргументы нормализуются сортировкой \
              по имени — иначе нарушается префикс-стабильность. POLICY \
              (allow=\"эфф1,эфф2\", require=\"confirmed\") — гейт декларации перед \
              вызовом, не изоляция: сверяет заявленные эффекты инструмента с \
              ALLOW, требует внешнего подтверждения при REQUIRE.",
        output_ebnf: None,
        stop_tokens: &[],
        normalized_slots: &["args", "policy"],
    });
